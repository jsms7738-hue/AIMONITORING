...
file_path = os.path.join(base_dir, "3월데이타 FT1.xlsx")
            ...
            for line in unique_lines:
                ...
                if l_n > 0:
                    ...
                    for _, f_row in nf.iterrows():
                        ...
                        # Check CHAR_IDs for failure location (heuristically, any non-empty CHAR_ID might be a step)
                        # In FT1, we look for CHAR_ID columns that have associated values
                        for i in range(1, 61):
                            cid_col = f'CHAR_ID_{i:02d}'
                            val_col = f'VALUE_{i:02d}'
                            if cid_col in f_row and pd.notna(f_row[cid_col]):
                                step_name = str(f_row[cid_col])
                                # Heuristic: if value is 0 or NaN or unusual, it might be the failing step
                                # But without clear spec, we just count all test items mentioned in failed rows
        ...
        for lid in sorted(lines_summary.keys()):
            ...
            for model_name in sorted(ms_raw.keys()):
                ...
                data["model_stats"].append({
                    ...
                    "fail": ms["fail"],
                    "pass_rate": pr,
                    "worst1_step": "" # FT1 doesn't have obvious step fail data in this view
                })
